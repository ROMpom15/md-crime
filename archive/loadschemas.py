...
# Check the row counts of the final merged DataFrames
...
print("\n--- Row Count Check for Merged DataFrames ---")
for name, df in dfs_to_check:
    # Triggering an action (.count()) forces the lazy union operations to execute
    row_count = df.count()
    
    status = "NO DATA" if row_count == 0 else "OK"
    
    # The :<12 ensures clean alignment for the names
    print(f"DataFrame: {name:<12} | Row Count: {row_count:<8} | Status: {status}")
